...
native gunxp_showmenu(id)
native moze_wybrac_bron(id)
native show_stats(id)
native rozgrzewka_on()
native moze_wybrac_bron_rozg(id)
native menu_pomocy(id)
...
show_menu_main(id)
{
	...
	// 6. Help
	len += formatex(menu[len], charsmax(menu) - len, "\r6.\w %L^n", id, "MENU_INFO")
	
	// 7. Stats
	len += formatex(menu[len], charsmax(menu) - len, "\r7.\r Statystyki^n^n", id)
