...
new
	...
	g_LSTAMMO,g_LACCESS,g_LGLOW,g_LDMGMODE,g_LCLMODE,g_LCBRIGHT,g_LDSEC,g_LCMDMODE,g_LBUYMODE,g_REWARDAP,g_REWARDXP;
...
public plugin_init() {
	...
	g_itemid_lminas = zp_items_register(g_item_name, g_item_lmines)
	// Add your code here...
	...
	g_REWARDAP	= register_cvar("zp_ltm_cmdaward_ap","2");		//nagroda za zniszczenie lm (AP)
	g_REWARDXP	= register_cvar("zp_ltm_cmdaward_xp","10");		//nagroda za zniszczenie lm (XP)
...
public fwHamTakeDamage( this, idinflictor, idattacker, Float:damage, damagebits ) {
	...
	if( ( equal( classname, ENT_CLASS_NAME )) && (!zp_core_is_zombie(idattacker)/* || idattacker == pev(this, pev_owner)*/))
		return HAM_SUPERCEDE;
	
	if( ( equal( classname, ENT_CLASS_NAME )) && zp_core_is_zombie(idattacker)){
//		new damage2 = hp_dzialka - floatround(damage)
//		change_health(this,-35,pev(ent,pev_iuser1),"grenade")
//		ExecuteHam(Ham_TakeDamage, this, 0, idattacker, damage, 1);
		new hp_miny
		hp_miny = pev(this, pev_health)
		client_print(idattacker, print_center, "HP Miny: %d", hp_miny) 
		if(damage >= hp_miny)
		{
			zp_ammopacks_set(idattacker, zp_ammopacks_get(idattacker) + get_pcvar_num(g_REWARDAP))
			set_user_xp(idattacker, get_user_xp(idattacker) + get_pcvar_num(g_REWARDXP))
//			client_print(idattacker, print_chat, "Dostales %i AP za zniszczenie LaserMiny", g_REWARD);
			new name[32]
			get_user_name(idattacker, name, 31)
			ColorChat(0,GREEN,"[ZM EXP]^x03 %s^x01 dostal^x04 %i^x01 AP oraz^x04 %i^x01 XP za zniszczenie LaserMiny", name, get_pcvar_num(g_REWARDAP), get_pcvar_num(g_REWARDXP));
			if(sprawdz_misje(idattacker) == 43)
				dodaj_ile_juz(idattacker, 1)
		}
		else if(hp_miny-damage > 0) client_print(idattacker, print_center, "HP Miny: %d", hp_miny-floatround(damage));
	}
...
public ReturnLaserMine_Progress(id)
{
	...
	set_task(3.0, "ReturnMine", (TASK_RELEASE + id));
...
public Spawn( id )
{
	...
	DeleteTask(id);
	//ShowAmmo(id);
	client_print(id, print_chat, "[ZP] LaserMine has been planted, You have %i LaserMines Remaining", g_havemine[id])
	return 1;
}
		...
		case BEAMBREAK_THINK :
		{
			...
			if ( fFraction < 1.0 )
			{
				...
				if(pev_valid(iHit))
				{
					...
					if( !equal( EntityName, ENT_CLASS_NAME ) &&  !equal( EntityName, gSnarkClassName ))
					{
...
CreateDamage(iCurrent,Float:DmgMAX,Float:Radius)
{
	...
	while((ent = engfunc(EngFunc_FindEntityInSphere, ent, vecSrc, Radius)) != 0)
	{
		...
		if (flFraction >= 0.9 || get_tr2(trRes, TR_pHit) == ent)
		{
			...
			{
				if(iHitTeam != TeamID || get_pcvar_num(g_LFF)  && !zp_get_user_zombie(AtkID) )
				{
					//set_pev(Player,pev_health,iHitHP)
...
public zp_fw_items_select_pre(id, itemid)
{
	if (itemid == g_itemid_lminas)
	{
		...
		if(zp_gamemodes_get_current() == zp_gamemodes_get_id("Swarm Mode") || zp_gamemodes_get_current() == ZP_NO_GAME_MODE || zp_gamemodes_get_current() == zp_gamemodes_get_id("Nemesis Mode") || zp_gamemodes_get_current() == zp_gamemodes_get_id("Assassin Mode") || zp_gamemodes_get_current() == zp_gamemodes_get_id("Dark Day Mode"))
...
public cmd_bind(id)
{
	if ( get_pcvar_num(g_LCMDMODE) == 1 )
	{
		if ( get_pcvar_num(g_BINDMODE) == 1 )
		{
			...
			client_print(id, print_chat, "[ZP] You have %i Laser Mines", g_havemine[id]+1)
...
public buy_lmines(id)
{
	...
	if ( !is_user_alive(id) ){	
		client_print(id,print_chat,"[ZP] Sorry, u are dead :)")
		return PLUGIN_HANDLED
	}
	...
	if ( zp_class_sniper_get(id) ){	
		client_print(id,print_chat,"[ZP] You can't buy LaserMines because You are Sniper!")
		return PLUGIN_HANDLED
	}
	
	if(zp_gamemodes_get_current() == ZP_NO_GAME_MODE){
		client_print(id,print_chat,"[ZP] Waititng for new Game Mode...")
		return PLUGIN_HANDLED
	}
	
	if (zp_gamemodes_get_current() == zp_gamemodes_get_id("Swarm Mode") || zp_gamemodes_get_current() == zp_gamemodes_get_id("Nemesis Mode") || zp_gamemodes_get_current() == zp_gamemodes_get_id("Assassin Mode") || zp_gamemodes_get_current() == zp_gamemodes_get_id("Dark Day Mode")){
		client_print(id,print_chat,"[ZP] You can't buy LaserMines on Custom Mode!")
		return PLUGIN_HANDLED
	}
	
	cmd_bind(id)
	new itemid = zp_get_extra_item_id("LaserMines")
	zp_force_buy_extra_item(id, itemid,0)
