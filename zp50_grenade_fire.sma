...
// Sprites
new g_sprite_grenade_trail[SPRITE_MAX_LENGTH] = "sprites/fire_trail.spr"
...
//    g_iFlare = precache_model("sprites/fire_gib.spr");
	g_trailSpr = precache_model("sprites/fire_trail.spr");
        g_ExploSpr = precache_model("sprites/fire_cannon.spr")  
}
...
create_blast2(const Float:origin[3])
{
	...
	message_end()
        ...
        new originF[3]
	originF[0] = floatround(origin[0])
	originF[1] = floatround(origin[1])
	originF[2] = floatround(origin[2])
	
/*	message_begin(MSG_BROADCAST, SVC_TEMPENTITY)
	write_byte(TE_SPRITETRAIL)
	write_coord(originF[0])
	write_coord(originF[1])
	write_coord(originF[2]+40)
	write_coord(originF[0])
	write_coord(originF[1])
