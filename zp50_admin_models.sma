...
new Array:g_models_human_claw

new cvar_admin_models_human_player, cvar_admin_models_human_knife
new cvar_admin_models_zombie_player, cvar_admin_models_zombie_knife, cvar_human_model_knife
...
public plugin_init()
{
	...
	cvar_human_model_knife = register_cvar("zp_human_models_knife", "1")
}
...
public plugin_precache()
{
	...
	g_models_human_claw = ArrayCreate(MODEL_MAX_LENGTH, 1)
	...
	amx_load_setting_string_arr(ZP_SETTINGS_FILE, "Weapon Models", "V_KNIFE HUMAN", g_models_human_claw)
	...
	if (ArraySize(g_models_human_claw) == 0)
	{
		for (index = 0; index < sizeof models_admin_zombie_claw; index++)
			ArrayPushString(g_models_human_claw, models_admin_zombie_claw[index])
		
		// Save to external file
		amx_save_setting_string_arr(ZP_SETTINGS_FILE, "Weapon Models", "V_KNIFE HUMAN", g_models_human_claw)
	}
	...
	for (index = 0; index < ArraySize(g_models_human_claw); index++)
	{
		ArrayGetString(g_models_human_claw, index, model, charsmax(model))
		precache_model(model)
	}
}
...
public zp_fw_core_cure_post(id, attacker)
{
	...
	if (get_pcvar_num(cvar_human_model_knife))
	{
		new model[MODEL_MAX_LENGTH]
		ArrayGetString(g_models_human_claw, random_num(0, ArraySize(g_models_human_claw) - 1), model, charsmax(model))
		cs_set_player_view_model(id, CSW_KNIFE, model)
	}
	// Skip if player doesn't have required admin flags
	if (!(get_user_flags(id) & read_flags(g_access_admin_models)))
