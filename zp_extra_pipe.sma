...
public plugin_init() {
	...
	if(get_pcvar_num(cvar_enabled))
		g_pipe = zp_items_register(g_extra_pipe, get_pcvar_num(cvar_cost))
//		zp_register_extra_item(g_extra_pipe, get_pcvar_num(cvar_cost), ZP_TEAM_HUMAN);
	
//	if(get_pcvar_num(cvar_Senabled))
//		g_mshield = zp_items_register(g_extra_shield, get_pcvar_num(cvar_Scost))
...
public native_daj_pipe(id)
{
	...
	{
		new msg[101], hud = random_num(0, 1);
	
		if(hud == 0)
			formatex(msg, 100, "%L", id, "DANGEROUS");
		else
			formatex(msg, 100, "%L", id, "GO_EXPLOSIVE");
...
public hook(entity) // Magnet func. Hooks zombies to nade
{
	...
	{
		if (!is_user_alive(victim) || !zp_core_is_zombie(victim) || g_has_shield[victim])
			continue;
...
public kill(const Float:originF[3], attacker) // Kills zombies in radius / 2 if mode = 1
{
	...
	while ((victim = engfunc(EngFunc_FindEntityInSphere, victim, originF, radius)) != 0)
	{
		...
		if(get_user_health(victim) <= 0)
		{
			...
			if(victim != attacker && !zp_core_is_zombie(attacker))
			{
				UpdateFrags(attacker, 1, 1);
				g_pointerxp = get_cvar_pointer( "gxm_xp" );
				new xp = get_pcvar_num(g_pointerxp)
				set_user_xp(attacker, get_user_xp(attacker) + xp)
				if(sprawdz_misje(attacker) == 40)
...
UpdateFrags(attacker, frags, scoreboard) // Updates attacker frags
{
	...
	if(get_pcvar_num(cvar_extraap) > 0)
		zp_ammopacks_set(attacker, zp_ammopacks_get(attacker) + get_pcvar_num(cvar_extraap));
