...
new UNCOUNTABLE_NADE_MODES 		=	((1 << (_:NADE_DUD + 1)) | (1 << (_:NADE_NORMAL + 1)) | (1 << (_:NADE_IMPACT + 1)))
...
new modetext[][] = { "Tryb: Normalny", "Tryb Natychmiastowy", "Tryb: Aktywacja [E]"}
...
public plugin_init()
{
	...
	register_option(OPTION_REMOVE_IF_DIES, "nademodes_remove_if_player_dies", "1")
	register_option(OPTION_AFFECT_OWNER, "nademodes_affect_owner", "0")
	register_option(OPTION_TEAM_PLAY, "nademodes_team_play", "0")
	...
	register_option(OPTION_TRIP_ENABLED, "nademodes_trip_enabled", "0")
	...
	register_option(OPTION_LIMIT_PROXIMITY, "nademodes_proximity_limit", "2", TOPTION_CELL)
	register_option(OPTION_LIMIT_TRIP, "nademodes_trip_limit", "2", TOPTION_CELL)
	register_option(OPTION_LIMIT_MOTION, "nademodes_motion_limit", "2", TOPTION_CELL)
	register_option(OPTION_LIMIT_SATCHEL, "nademodes_satchel_limit", "3", TOPTION_CELL)
...
public event_armnade(id)
{
	...
	{
		client_print(id, print_center, "%s", modetext[_:mode[id][NadeRace:nade]])
	}
...
public fw_cmdstart(id, uc_handle, seed)
{
	...
	{
		if (!key[id] && !zp_core_is_zombie(id))
		{
			switch (cl_weapon[id])
			{
...
grenade_process(id, grenade, NadeRace:nade)
{
	if ((cl_is_bot & (1<<id)) && get_option(OPTION_BOT_ALLOW) && grenade_can_be_used(nade))
	{
		...
		static const NadeType:random_vec[] = {NADE_NORMAL, NADE_IMPACT}
...
public fw_think(ent)
{
	...
	if ((g_zombie_mod & ZM_ZM_ACTIVE) && !(g_zombie_mod & ZM_CAN_THINK) && (TRIP_ATTACHED <= get_trip_grenade_mode(ent) <= TRIP_WAITING))
		return HAM_IGNORED
...
stock NadeType:FirstEnabledMode(NadeRace:race)
{
	...
	for (new NadeType:i=NADE_NORMAL;i<=NADE_SATCHEL;i++)
	{
...
public is_mode_enabled(id, NadeType:type, NadeRace:nade)
{
	switch (type)
	{
		...
		case NADE_SATCHEL:
		{
			...
			if (get_option(OPTION_LIMIT_SYSTEM) == 2 && (cl_counter[id][GRENADE_EXPLOSIVE][type] + cl_counter[id][GRENADE_SMOKEGREN][type] + cl_counter[id][GRENADE_FLASHBANG][type]) <= 2)
				return 1
...
{
	for (new NadeType:type = NADE_NORMAL; type <= NADE_SATCHEL; type = type + NADE_IMPACT)
	{
...
public changemode(id, NadeRace:NADE_TYPE)
{
	...
	switch (mode[id][NADE_TYPE])
	{
		case NADE_NORMAL:
		{
			client_print(id, print_center, "Tryb: Normalny")
		}
		
		case NADE_IMPACT:
		{
			client_print(id, print_center, "Tryb: Natychmiastowy")
		}
		
		case NADE_SATCHEL:
		{
			client_print(id, print_center, "Tryb: Aktywacja [E]")
		}
