...
#define HUMANS_DEFAULT_ARMOR 10
...
public plugin_init()
{
	...
	register_clcmd("say /ludzie", "show_class_menu")
...
{
	precache_model("models/player/gold_model_fifka/gold_model_fifka.mdl")
	precache_model("models/player/gold_model_fifka/gold_model_fifkaT.mdl")
}
...
public plugin_natives()
{
	...
	g_HumanClassDesc = ArrayCreate(201, 1)
...
{
	static menu[128], name[32], description[201], transkey[64]
...
public menu_humanclass(id, menuid, item)
{
	...
	zp_colored_print(id, "%L: %d %L: %d %L: %.2f %L: %d", id, "ZOMBIE_ATTRIB1", ArrayGetCell(g_HumanClassHealth, g_HumanClassNext[id]), id, "ZOMBIE_ATTRIB2", cs_maxspeed_display_value(maxspeed), id, "ZOMBIE_ATTRIB3", Float:ArrayGetCell(g_HumanClassGravity, g_HumanClassNext[id]), id, "ZOMBIE_ATTRIB5", ArrayGetCell(g_HumanClassArmor, g_HumanClassNext[id]))
...
public zp_fw_core_cure_post(id, attacker)
{
	...
	if (class_models != Invalid_Array)
	{
		...
		if(get_user_level(id) > 23)
			cs_set_player_model(id, "gold_model_fifka")
		else cs_set_player_model(id, player_model)
	}
	else
	{
