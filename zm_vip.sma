...
public print_adminlist(user) 
{
	...
	len = format(message, 255, "Vipy w grze: ", id)
	if(count > 0) {
		for(x = 0 ; x < count ; x++) {
...
public client_PreThink(id)
{
	...
	if((nbut & IN_JUMP) && !(fflags & FL_ONGROUND) && !(obut & IN_JUMP))
	{
		...
		((g_bit & FLAG_D && (zp_get_user_nemesis(id) || zp_class_assassin_get(id))) || 
		(g_bit & FLAG_C && (zp_get_user_survivor(id) || zp_class_sniper_get(id))) || 
		(g_bit & FLAG_A && !zp_get_user_zombie(id) && !zp_get_user_survivor(id) && !zp_class_sniper_get(id)) ||
		(g_bit & FLAG_B && zp_get_user_zombie(id) && !zp_get_user_nemesis(id) && !zp_class_assassin_get(id))))
		{
			dojump[id] = true
			jumpnum[id]++
