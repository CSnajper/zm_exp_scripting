...
new cvar_ammop_nemesis_ignore, cvar_ammop_survivor_ignore, cvar_ammop_zombie_die_chance
...
public plugin_init()
{
	...
	cvar_ammop_zombie_die_chance = register_cvar("zp_ammop_zombie_die_chance", "2")
...
public fw_PlayerKilled_Post(victim, attacker, shouldgib)
{
	...
	if(get_pcvar_num(cvar_ammop_zombie_die_chance) && zp_core_is_zombie(victim))
	{
		if(random_num(1, get_pcvar_num(cvar_ammop_zombie_die_chance) == 1))
		{
			zp_ammopacks_set(victim, zp_ammopacks_get(victim) + 1)
			zp_colored_print(victim, "Dostales ^x04 1 ^x01 AP za probe zarazenia")
		}
	}
}
