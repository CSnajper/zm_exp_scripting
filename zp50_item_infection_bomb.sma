...
{
					// Player natives //
	register_native("daj_bombe", "native_daj_bombe", 1);
}

public native_daj_bombe(id)
{
	// Give infection bomb
...
infection_explode(ent)
{
	...
	while ((victim = engfunc(EngFunc_FindEntityInSphere, victim, origin, NADE_EXPLOSION_RADIUS)) != 0)
	{
		...
		if (zp_core_get_human_count() == 1 || zp_core_is_last_human(victim))
		{
