...
new cvar_knockback_nemesis, cvar_knockback_nemesis_rozg
...
public plugin_init()
{
	...
	if (LibraryExists(LIBRARY_NEMESIS, LibType_Library))
	{
		...
		cvar_knockback_nemesis_rozg = register_cvar("zp_knockback_nemesis_rozgrzewka", "0.01")
	}
}
...
public fw_TraceAttack_Post(victim, attacker, Float:damage, Float:direction[3], tracehandle, damage_type)
{
	...
	if (LibraryExists(LIBRARY_NEMESIS, LibType_Library) && zp_class_nemesis_get(victim))
	{
		...
		if(rozgrzewka_on())
			xs_vec_mul_scalar(direction, get_pcvar_float(cvar_knockback_nemesis_rozg), direction)
		else xs_vec_mul_scalar(direction, get_pcvar_float(cvar_knockback_nemesis), direction)
	}
