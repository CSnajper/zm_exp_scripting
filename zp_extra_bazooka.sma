...
public plugin_init() {
	...
	g_item_jetpack = zp_items_register("Bazooka", JETPACK_COST)
...
{
					// Player natives //
	register_native("daj_bazooke", "native_daj_bazooke", 1);
}

public native_daj_bazooke(id)
{
	hasjet[id] = 1
...
public check_models(id) {
	...
	if (zp_get_user_zombie(id) || zp_get_user_nemesis(id) || zp_get_user_survivor(id) || zp_class_sniper_get(id))
		return FMRES_IGNORED
...
public rk_sound(player) {
	...
	if (!rksound[player])
	{
		...
		client_print(player, print_center, "[Bazooka] Bazooka Przeladowana")
		rksound[player] = true
	}
...
public zp_fw_items_select_post(id, itemid, ignorecost)
{
	...
	if (itemid == g_item_jetpack)
	{
		client_print(id, print_chat, "[ZM EXP] Kupiles Bazooke! Aby z niej skorzystac wyciagnij noz, oddaj strzal za pomoca PPM.")
		hasjet[id] = 1
		g_flyEnergy[id] = get_pcvar_num(cvar_fly_max_engery)
		rocket[id] = true
		client_cmd(id,"spk items/gunpickup2.wav")
