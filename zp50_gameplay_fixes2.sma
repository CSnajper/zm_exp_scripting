...
public client_disconnect(leaving_player)
{
	...
	else
	{
		...
		if (zp_core_is_zombie(leaving_player) && zp_core_get_zombie_count() == 1)
		{
			...
			if (zp_gamemodes_get_current() == NemesisID && ma_nemka[leaving_player] == 1/* && zp_class_nemesis_get(leaving_player)*/)
			{
				client_print(id,print_chat,"Odpalony warunek");
				zp_class_nemesis_set(id)
				client_print(id,print_chat,"Powinno dac nemka");
				if (get_pcvar_num(cvar_keep_hp_on_disconnect))
					set_user_health(id, get_user_health(leaving_player))
				client_print(id,print_chat,"Powinno dac HP");
			}
			else{
				zp_core_infect(id, id)
				client_print(id,print_chat,"Jednak ustawilo zwyklego zombiaka");
			}
		}
		// Last Human
